...
paragraphs that retain exactly the same sampled parameters. It then runs
both evaluation arms on every wording and scores consistency by exact
equality of the arms' extracted actions. This is deliberately separate
from accuracy: the answer key is not used when computing consistency.
"""
...
def _action_is_consistent(actions: list) -> bool:
    """Return true only when every wording yielded the exact same action.

    ``None`` is retained as an extracted outcome rather than discarded, so
    a run is consistent if all variants parse-failed identically, and
    inconsistent if some variants parse while others fail.
    """
    return len(actions) > 0 and len(set(actions)) == 1


def _is_connection_error(raw_response: Optional[str]) -> bool:
    """Detect calls that never reached the model at all (vs. a real reply
    the parser simply couldn't extract an action from)."""
    return bool(raw_response) and raw_response.startswith("[ERROR calling")


def aggregate_consistency(scenario_results: list) -> dict:
    ...
    failed to connect to the model) are excluded from both the numerator
    and denominator -- they are not evidence of consistency OR
    inconsistency, so counting them either way would bias the rate.
    """
...
def run_consistency_check(
    ...
    *,
    ...
    Returns ``(report, per_scenario_results, raw_records)``. ``raw_records``
    is also written as JSONL, one row per scenario/arm/wording, with the
    paragraph and normalized extracted answer retained for manual
    disagreement spot checks. ``per_scenario_results`` contains the
    required Boolean ``consistent`` flag for each scenario and arm.
    """
    if sample_size < 1:
    ...
    # come back as connection-error contamination. Only applies when using
    # the real Ollama-backed runners -- tests that inject mocked runners
    # (and a dummy client) intentionally skip this.
    ...
    for scenario_id in range(sample_size):
        ...
        for wording_id, paragraph in enumerate(paragraphs, start=1):
            try:
                ...
                direct = {"action": None, "raw_response": f"[ERROR calling Direct-AI: {type(exc).__name__}: {exc!r}]"}
            ...
            try:
                ...
                translator = {"action": None, "raw_response_text": f"[ERROR calling Translator-plus-Solver: {type(exc).__name__}: {exc!r}]"}

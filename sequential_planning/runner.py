...
def run_evaluation(n: int, log_path: str = "run_log.jsonl", seed: int = None, sleep_between_calls: float = 0.0):
    ...
    for i in range(n):
        ...
        try:
            ...
            direct_result = {"action": None, "raw_response": f"[ERROR calling Direct-AI: {type(e).__name__}: {e!r}]"}
        ...
        try:
            ...
            translator_result = {
                ...
                "extraction_warnings": [f"ERROR calling Translator-plus-Solver: {type(e).__name__}: {e!r}"],
                "raw_response_text": "",
                "solver_detail": None,
            }
        ...
        print(f"[{i+1}/{n}] truth={scored['ground_truth']:<11} "
              f"direct={str(scored['direct_action']):<12}({status_direct:<5}) "
              f"translator={str(scored['translator_action']):<12}({status_translator})")
